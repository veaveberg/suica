...
export function SpaceShareDropdown({ onPreparePost, space }: Props) {
    ...
    return <details ref={detailsRef} className="relative z-50">
        <summary aria-label={t('space_share_availability')} className="list-none cursor-pointer rounded-lg p-2 text-ios-gray active:bg-black/5 dark:active:bg-white/10 [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-0.5"><Share2 className="h-4 w-4" /><ChevronDown className="h-3 w-3 text-ios-gray" /></span></summary>
