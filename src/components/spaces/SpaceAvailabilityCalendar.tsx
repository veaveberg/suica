...
interface Props { content?: SpaceCalendarContent; days: AvailabilityDay[]; events?: ManagedSpaceEvent[]; eventColor?: string; futureAvailabilityOnly?: boolean; mode: SpaceCalendarMode; onModeChange: (mode: SpaceCalendarMode) => void; onPeriodChange?: (period: string) => void; showNowLine?: boolean; timeZone: string; workingHours: WorkingHours; showToolbar?: boolean; }
...
export const SpaceAvailabilityCalendar = memo(function SpaceAvailabilityCalendar({ content = 'availability', days, events = [], eventColor = '#f472a0', futureAvailabilityOnly = false, mode, onModeChange, onPeriodChange, showNowLine = true, timeZone, workingHours, showToolbar = true }: Props) {
    ...
    return <div className="relative flex h-full min-h-0 flex-col bg-ios-background dark:bg-black">
        ...
        {mode === 'month' ? <div ref={monthRef} className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain pb-32 hide-scrollbar">
            ...
            <div className="relative" style={{ height: monthContentHeight }}>{visibleMonthWeeks.map(({ height: weekHeight, index: weekIndex, range, top, week }) => {
                ...
                const showCurrentTime = showNowLine && isCurrentWeek && range !== null && currentMinutes >= range.start && currentMinutes <= range.end;
            ...
            })}{showNowLine && <div className={cn('pointer-events-none absolute left-0 right-0 z-10', isToday ? 'h-0.5 bg-ios-red' : 'h-px')} style={{ top: minutesInTimeZone(currentTime.getTime(), timeZone) * PIXELS_PER_MINUTE, backgroundColor: isToday ? undefined : 'rgb(255 59 48 / 0.45)' }}>{isToday && <span className="absolute -left-1 -top-[3px] h-2 w-2 rounded-full bg-ios-red" />}</div>}</div></section>;
        })}</div></div>}
